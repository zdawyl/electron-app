...
const ExpoetSetThemeColor = () => {
  try {
    let pptitem = document.getElementById('ppt2')
    let itemcon = pptitem.getElementsByClassName('content')[0]
    let bg = getComputedStyle(itemcon)?.backgroundColor
    let objArr = ['#000000', '#ffffff']
    if (bg) {
      let color = chroma(bg)
      let fontColor = color.luminance() > 0.5 ? '#000000' : '#ffffff'
      let fontColor2 = fontColor === '#000000' ? '#ffffff' : '#000000'
      objArr = [fontColor, fontColor2]
    }
    objArr.forEach((item, index) => {
      document.documentElement.style.setProperty(`--edit-${index}`, item)
    })
    return objArr
  } catch (error) {
...
function getFontColor(colorArr) {
  let fontColorArr = []
  colorArr.forEach((item) => {
    let color = chroma(item)
    let fontColor = color.luminance() > 0.5 ? '#000' : '#fff'
    fontColorArr.push(fontColor)
  })
  return fontColorArr
}
...
function rgb(r, g, b) {
  ...
  b = rangeLimit(b)
  let [o, p, q] = [r, g, b]
    .map((item) => Math.round(item).toString(16))
    .map((item) => {
...
export function colorPalette(hex) {
  ...
  let res = getFontColor(palette)
  let req = ExpoetSetThemeColor()
  return [...palette, ...res, ...req]
}
